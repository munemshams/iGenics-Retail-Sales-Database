...
def plot_weekly_total_metric(clean: pd.DataFrame, metric_name: str, outpath: str):
    ...
    grp = df.groupby(["year", "week"], as_index=False)["value"].sum().sort_values(["year", "week"])
    labels = grp["year"].astype(int).astype(str) + "-W" + grp["week"].astype(int).astype(str).str.zfill(2)
...
def main():
    ...
    # Calculate resume-friendly metrics
    weeks_total = clean[["year", "week"]].drop_duplicates().shape[0]
    weeks_by_year = clean[["year", "week"]].drop_duplicates().groupby("year").size().to_dict()

    revenue = clean[clean["metric_name"].str.lower() == "revenue"].groupby("year")["value"].sum().to_dict()
    total_net = clean[clean["metric"].str.lower() == "total net income"].groupby("year")["value"].sum().to_dict()
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"- Total weeks: {weeks_total}\n")
        f.write(f"- Weeks by year: {weeks_by_year}\n")
        f.write(f"- Total revenue by year: {revenue}\n")
        f.write(f"- Total net income (Total Net Income): {total_net}\n")
